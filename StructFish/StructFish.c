#include <stdio.h>
//Пример вложенных структур и как добавить данные в такую структуру а так же как вывести через функцию

struct exercise {         // Струткра exercise имеет два атребута description и duration
	const char* description;
	float duration;
};

struct meal {            // Структура meal имеет два атрибута ingredients и weight
	const char* ingredients;
	float weight;
};

struct preferences { //структуры meal и exercise вложены в структуру preferences и имеют названия food и exercise
	struct meal food;
...
label(struct fish a)
{
	...
	// пример как вывести данные через функцию которая обращается к вложенным структурам
}
