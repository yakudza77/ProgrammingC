...
int main()
{
	char line[80]; //for work C:/Users/o.kostea/Desktop/Programmtest/
	...
	FILE* file2 = fopen("D:/TESTPROGRAMM/disappearances.csv", "w");//w записывает в файл disappearances.csv использу€ поток file2
	FILE* file3 = fopen("D:/TESTPROGRAMM/others.csv", "w");//w записывает в файл others.csv использу€ поток file3

	while (fscanf(in, "%79[^\n]\n", line) == 1) { //считываетс€ построчно из потока in и отправл€ет данные в массив line 
	...
	fclose(file1);// закрываем потоки после завершени€ работы цикла while
	fclose(file2);
	fclose(file3);
	return 0;
